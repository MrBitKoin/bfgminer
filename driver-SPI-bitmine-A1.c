...
{
	spi_clear_buf(spi_ctx->port);
	if (tx)
		spi_emit_buf(spi_ctx->port, tx, len);
	else
		spi_emit_nop(spi_ctx->port, len);
	if (!spi_txrx(spi_ctx->port))
		return false;
	memcpy(rx, spi_ctx->port->spibuf_rx, len);
	return true;
}
...
#define ASIC_SPI_CS0_GPIO  14
#define ASIC_SPI_CS1_GPIO  15
#define ASIC_SPI_CS2_GPIO  18
#define ASIC_SPI_EN_GPIO   25
...
int gpio_CS_Init(void)
{
	...
	bfg_gpio_setpin_output(ASIC_SPI_EN_GPIO);
	bfg_gpio_set_low(1<<ASIC_SPI_EN_GPIO);
	
	bfg_gpio_setpin_output(ASIC_SPI_CS0_GPIO);
	bfg_gpio_set_low(1<<ASIC_SPI_CS0_GPIO);
	
	bfg_gpio_setpin_output(ASIC_SPI_CS1_GPIO);
	bfg_gpio_set_low(1<<ASIC_SPI_CS1_GPIO);
	
	bfg_gpio_setpin_output(ASIC_SPI_CS2_GPIO);
	bfg_gpio_set_low(1<<ASIC_SPI_CS2_GPIO);
